/**
 * One-off cleaner for new_toUPLOAD_psychiatrist.xlsx — writes a cleaned copy + JSON report.
 * Run: node scripts/clean-psychiatrist-spreadsheet.mjs [input.xlsx] [output.xlsx]
 *
 * "Clean" = normalize names / specialty for import — not delete professionals.
 */
...
const inputPath =
  process.argv[2] ??
  path.join(process.env.USERPROFILE ?? "", "Downloads", "new_toUPLOAD_psychiatrist.xlsx");
const outputPath =
  process.argv[3] ??
  path.join(process.env.USERPROFILE ?? "", "Downloads", "new_toUPLOAD_psychiatrist_CLEANED.xlsx");
const reportPath = outputPath.replace(/\.xlsx$/i, "_report.json");

/** Exact raw-name fixes (surname-first → given-name-first). Do not reorder Western given+surname. */
const EXACT_NAME_OVERRIDES = new Map([
  ["Fragkou Martha", "Martha Fragkou"],
  ["Ioannou Giannis", "Giannis Ioannou"],
  ["Hadjivasilis Vasilis", "Vasilis Hadjivasilis"],
  ["Josephides Stephen", "Stephen Josephides"],
  ["Papaneofytou Antonios", "Antonios Papaneofytou"],
  ["Aspris Nikos", "Nikos Aspris"],
  // Preserve intentional internal capital (avoid Title Case flattening).
  ["Andreas ChatziAntonas", "Andreas ChatziAntonas"],
]);
...
function cleanPsychiatristName(rawName) {
  ...
  if (EXACT_NAME_OVERRIDES.has(collapsed)) {
    const cleaned = EXACT_NAME_OVERRIDES.get(collapsed);
    return {
      cleaned,
      action: cleaned === collapsed ? "unchanged" : "override",
    };
  }
...
const report = {
  ...
  keptRows: 0,
  specialtyNormalizedTo: "Psychiatry",
  skippedClinic: [],
  nameChanges: [],
  unchangedNames: 0,
  noPhone: [],
  ...
  byDistrict: {},
  notes: [
    "Clean = normalize for import (names, specialty), not delete people.",
    "psychiatrist → Psychiatry",
    "Kept multi-consultation duplicate names with different Maps links",
    "Did not reorder Marilyn George / Iro Michael / Eleni Michael",
  ],
};
...
for (const [index, row] of rows.entries()) {
  ...
  const { cleaned, action } = cleanPsychiatristName(rawName);
  ...
  for (const key of Object.keys(cleanedRow)) {
    ...
    if (lower === "specialty" || lower === "speciality") {
      cleanedRow[key] = "Psychiatry";
      hasSpecialty = true;
    }
  }
  if (!hasName) cleanedRow.name = cleaned;
  if (!hasSpecialty) cleanedRow.specialty = "Psychiatry";
...
console.log(`Duplicate names kept: ${report.duplicateNamesInFile.length}`);
