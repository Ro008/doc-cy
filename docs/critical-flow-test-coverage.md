...
| UserBar (desktop dropdown + mobile tabs) | `pro_sticky_header.spec.ts` | `practice_insights.spec.ts`, `navigation_feedback.spec.ts` | Sticky header on `/agenda*` (desktop); dropdown open/close; mobile tabs |
...
| Flow | PR blocking | Other | Notes |
| --- | --- | --- | --- |
| Doctor onboarding (register → founder alert → approve → doctor email → agenda) | `doctor_onboarding_pipeline.integration.spec.ts` (`@pr-e2e`) + `doctor_onboarding_emails.integration.spec.ts` (`@pr-email`) | `doctor_register_flow.integration.spec.ts` + `doctor_register_claim_flow.integration.spec.ts` (`@local-register`) via `npm run test:e2e:register` before opening a PR. Agenda login covered on PR integration. | PR uses `createTestDoctor` as post-registration DB state. Live `/register` + Resend (including claim from a QA clone public profile) is a **local pre-PR gate**, not GitHub CI. Email copy on PR; Resend send on verify is runtime best-effort. `doctor_account_access.integration.spec.ts` covers reject/edge cases. |
...
1. **High — Doctor chrome after login:** covered by `pro_sticky_header.spec.ts` (desktop sticky header + dropdown) and partial via `practice_insights.spec.ts` + `navigation_feedback.spec.ts` (mobile tabs; More menu; no mobile manual-booking FAB).
...
1. ~~**UserBar smoke (signed-out only)**~~ — **Done** in `pro_sticky_header.spec.ts` (signed-out landing + authenticated sticky/dropdown/mobile). Extend only if new routes need chrome rules.
