...
| Flow | PR blocking | Other | Notes |
| --- | --- | --- | --- |
| Doctor onboarding (register → founder alert → approve → doctor email → agenda) | `doctor_onboarding_pipeline.integration.spec.ts` (`@pr-e2e`) + `doctor_onboarding_emails.integration.spec.ts` (`@pr-email`) | `doctor_register_flow.integration.spec.ts` (`@local-register`) via `npm run test:e2e:register` before opening a PR. Agenda login covered on PR integration. | PR uses `createTestDoctor` as post-registration DB state. Live `/register` + Resend is a **local pre-PR gate**, not GitHub CI. Email copy on PR; Resend send on verify is runtime best-effort. `doctor_account_access.integration.spec.ts` covers reject/edge cases. |
