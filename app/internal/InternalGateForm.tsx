"use client";

import * as React from "react";
import { useSearchParams } from "next/navigation";

export function InternalGateForm() {
  const searchParams = useSearchParams();
  const [password, setPassword] = React.useState("");
  const [error, setError] = React.useState<string | null>(null);
  const [loading, setLoading] = React.useState(false);

  const configure = searchParams.get("configure") === "1";
  const nextPath = searchParams.get("next") || "/internal/directory";

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    setLoading(true);
    try {
      const res = await fetch("/api/internal/auth", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ password }),
        credentials: "same-origin",
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setError((data.message as string) || "Access denied.");
        return;
      }
      // Full navigation so the Set-Cookie from the previous response is always sent
      // on the next request (avoids races with client-side router.push).
      window.location.assign(nextPath.startsWith("/internal") ? nextPath : "/internal/directory");
    } catch {
      setError("Something went wrong.");
    } finally {
      setLoading(false);
    }
  }

  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 text-slate-50">
      <div className="w-full max-w-md rounded-3xl border border-slate-800 bg-slate-900/80 p-8 shadow-xl">
        <h1 className="text-xl font-semibold text-slate-50">Internal access</h1>
        <p className="mt-2 text-sm text-slate-400">
          Enter your founder or business partner access code to continue.
        </p>

        {configure ? (
          <div className="mt-4 rounded-xl border border-amber-500/40 bg-amber-500/10 px-3 py-3 text-sm text-amber-100/95">
            <p className="font-medium text-amber-50">Directory not configured</p>
            <p className="mt-1 text-amber-100/90">
              Add{" "}
              <code className="rounded bg-black/30 px-1 text-xs">INTERNAL_DIRECTORY_SECRET</code>{" "}
              to <code className="rounded bg-black/30 px-1 text-xs">.env.local</code>, pick any
              strong passphrase, then <strong>restart</strong>{" "}
              <code className="rounded bg-black/30 px-1 text-xs">npm run dev</code>. Use that same
              value below as the access code. Optional:{" "}
              <code className="rounded bg-black/30 px-1 text-xs">INTERNAL_DIRECTORY_PARTNER_SECRET</code>{" "}
              for a distinct read-only business partner password.
            </p>
          </div>
        ) : null}

        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <input
            type="password"
            autoComplete="current-password"
            value={password}
            onChange={(e) => setPassword(e.target.value)}
            className="w-full rounded-xl border border-slate-700 bg-slate-950/60 px-3 py-2.5 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-clinical-500/40"
            placeholder="Access code"
          />
          {error ? (
            <p className="text-sm text-red-300">{error}</p>
          ) : null}
          <button
            type="submit"
            disabled={loading || !password.trim()}
            className="w-full rounded-xl bg-clinical-500 py-2.5 text-sm font-semibold text-slate-950 transition hover:bg-clinical-400 disabled:opacity-50"
          >
            {loading ? "Checking…" : "Continue"}
          </button>
        </form>
      </div>
    </main>
  );
}
